...
export function TodoList() {
  ...
  useEffect(() => {
    console.log('TodoList useEffect:', {isConnected, hasProvider: !!walletProvider, address});
    setError(null);
  ...
  const loadTodos = async () => {
    ...
    try {
      ...
      if (error.message.includes('timeout') || error.message.includes('network')) {
        ...
        // This often happens with new contracts or network issues
        // Let's just show empty state instead of error for this case
      ...
      // Set empty todos array so we don't show old data
      setTodos([]);
    } finally {
      clearTimeout(timeoutId);
      setLoading(false);
